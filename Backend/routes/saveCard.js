...
router.post("/", async (req, res) => {
  try {
    const { cardName, cardType, cardNumber, nameOnCard, expiryMonth, expiryYear, cvv } = req.body;
    const newCard = new SCard({ cardName, cardType, cardNumber, nameOnCard, expiryMonth, expiryYear, cvv });
    await newCard.save();
...
router.put("/:id", async (req, res) => {
  try {
    const { cardName, cardType, cardNumber, nameOnCard, expiryMonth, expiryYear, cvv } = req.body;
    const updatedCard = await SCard.findByIdAndUpdate(req.params.id, 
